...
var UserView = React.createClass({
  render: function() {
    var name = this.props.user.get('role') == 'guest' ? 'guest' : this.props.user.get('user');

    return React.DOM.div({className: '.lifemagazine-chat-left-img', style: {top: '0px', left: '10px', width:'30px', height: '30px', position: 'absolute'}},
      React.DOM.img({src: this.props.user.image(name), style: {width:'30px', height: '30px', border: '0px'}, title: this.props.user.get('user')}),
      null
    )
  }
});
